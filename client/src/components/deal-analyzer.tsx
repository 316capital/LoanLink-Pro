...
import { Calculator, TrendingUp, DollarSign, Percent, Lock, CheckCircle, AlertTriangle, ArrowRight, Zap } from "lucide-react";
...
export function DealAnalyzer() {
  ...
  const getTierInfo = (credit: string, exp: string, desiredLtcPct: number) => {
    ...
    const expNum = exp === "5+" ? 5 : exp === "3-5" ? 3 : exp === "1-2" ? 1 : 0;

    // Check for 100% - Vault only
    if (desiredLtcPct === 100) {
      if (creditNum >= 700 && expNum >= 3) {
        return { maxLtc: 100, maxArvLtv: 70, rate: 9.99, points: 2, tier: "Vault Elite", requiresVault: true };
      }
